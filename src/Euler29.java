    ...
    public static void main(String[] args)
    {
        ...
        for(int i = start; i <= end; i++)
        {
            for(int j = start; j <= end; j++)
            {
                ...
                if(!uniqueNumbers.contains(number.toString()))
                   uniqueNumbers.add(number.toString());
            }
        }
        Collections.sort(uniqueNumbers);
        System.out.println(" Distinct terms are in the sequence generated by ab for 2 ≤ a ≤ 100 and 2 ≤ b ≤ 100 are " + uniqueNumbers.size());
    }
}
